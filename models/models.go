...
type TRelation struct {
	...
	State     uint8       `json:"state" gorm:"tinyint;default:1" remark:"关系的状态,1表示发起中,0表示双向已建立,2表示单向(己方->对方)已建立"`
	CreatedAt time.Time   `json:"created_at,omitempty"`
	UpdatedAt time.Time   `json:"updated_at,omitempty"`
	Status 	  uint8		  `json:"status,omitempty" gorm:"tinyint;default:0" remark:"数据的状态,0表示存在,1表示已删除,可扩展"`
}
...
type Message struct {
	...
	Type      uint8 	  `json:"type" remark:"消息类型,0普通消息(对单),1加好友(对单),2群消息,3加群,4搜索用户"`
}