...
type TRelation struct {
	...
	State     uint8       `json:"state,omitempty" gorm:"tinyint;default:1" remark:"关系的状态,1表示发起中,0表示双向已建立,2表示单向(己方->对方)已建立"`
	CreatedAt time.Time   `json:"created_at,omitempty"`
...
type TMessage struct {
	...
	Type      uint8 	  `json:"type,omitempty" gorm:"tinyint" remark:"消息类型,0普通消息(对单),1加好友(对单),2群消息,3加群"`
...
type Message struct {
	...
	Type      uint8 	  `json:"type,omitempty" remark:"消息类型,0普通消息(对单),1加好友(对单),2群消息,3加群"`
}